...
class VGG19(torch.nn.Module):
    def __init__(self, device='cpu'):
        ...
        features = list(models.vgg19(pretrained=True).features)  # Use models.vgg19 here
        if device == "cuda":
...
def segmentation():
    global image_counter  # Access the global counter
    ...
    # Save the segmented image with a unique name based on the counter
    save_folder = "F:/PROJECTS/Major Project/Sem - VII/Major Project final/Webpage 2/static/uploads"  # Specify your folder path here
    os.makedirs(save_folder, exist_ok=True)  # Create the folder if it doesn't exist
    image_filename = f"segmented_{image_counter}.jpg"  # Unique filename using counter
    cv2.imwrite(os.path.join(save_folder, image_filename), im1)
